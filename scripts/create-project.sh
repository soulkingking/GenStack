...
usage() {
  cat >&2 <<'EOF'
用法: bash scripts/create-project.sh <project-name> [target-dir] [--skip-install]
...
install_dependencies() {
  ...
  bash "$TARGET/scripts/init-frontend.sh" || return 1
}

if [[ "$SKIP_INSTALL" == true ]]; then
  echo "完成（已跳过依赖安装）。后续步骤（需 Python 3.11 与 Node.js >= 22）:"
  print_manual_steps
elif install_dependencies; then
  echo "完成，前后端依赖已安装。"
else
  echo "完成，但依赖安装未成功，请手动执行（需 Python 3.11 与 Node.js >= 22）:" >&2
  print_manual_steps
fi
